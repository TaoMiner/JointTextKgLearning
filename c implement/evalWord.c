...
int main(int argc, char **argv) {
    ...
    if(0!=word.read_file[0]&&0!=eval_file[0]){
        ...
        printf("Hit@%d in %d/%d analogy pairs precison %f, skip %d pairs, comment %d lines.", N, correct_num, line_count, (float)correct_num/(line_count-comment_line_num-skip), skip, comment_line_num);
    }
    return 0;
}
