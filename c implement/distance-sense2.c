...
int SearchMentionVocab(char *item) {
    ...
    while (1) {
        ...
        if (!strcmp(item, &mention.vocab[mention.vocab_hash[hash]*MAX_STRING])) return mention.vocab_hash[hash];
        hash = (hash + 1) % vocab_hash_size;
    }
    return -1;
}
...
void FindNearest(int top_n, int id){
    char *bestw[top_n];
...
//    for (a = 0; a < size; a++) vec[a] /= len;
    ...
    int mention_id = -1;
    mention_id = word.mention_index[id];
    if(mention_id==-1){
        printf("the input is not a mention!");
        return;
    }
    printf("there are %d senses.", mention.sense_num[mention_id]);
    for (s = 0; s < mention.sense_num[mention_id]; s++){
        for (a = 0; a < size; a++) vec[a] = 0;
        for (a = 0; a < size; a++) vec[a] += mention.M[a + mention_id *mention.max_sense_num * size+s*size]];

        printf("\n                                              sense      Cosine distance\n------------------------------------------------------------------------\n");
