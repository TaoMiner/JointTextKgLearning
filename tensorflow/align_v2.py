...
_kg_skipgram_module=tf.load_op_library('./custom_op/kg_skipgram.so')
kg_skipgram=_kg_skipgram_module.k_gskipgram
# align model op for build the anchor example
_align_model_module=tf.load_op_library('./custom_op/align_model.so')
...
flags.DEFINE_string("load_path", None, "Directory to load the model.")
flags.DEFINE_string(
    "text_data", None,
...
class Word2Vec(object):
  ...
  def __init__(self, options, session):
    ...
    #self._vocab_size_limit = 512000000/self._options.emb_dim-1
    self._vocab_size_limit = 100000
  def build(self):
    self.build_graph()
  ...
  def build_graph(self):
    ...
    if self._is_loaded:
      ...
      # 2048/4-1 mb
      if opts.vocab_size > self._vocab_size_limit:
        self._is_shard = True
        remain_size_ = opts.vocab_size
        i = 0
        while remain_size_>0 :
          if remain_size_ > self._vocab_size_limit:
            self._init_v_in.append(tf.Variable(tf.random_uniform([self._vocab_size_limit,opts.emb_dim], -0.5 / opts.emb_dim, 0.5 / opts.emb_dim),name="v_in"+str(i)))
            self._init_v_out.append(tf.Variable(tf.zeros([self._vocab_size_limit,opts.emb_dim]),name="v_in"+str(i)))
            remain_size_ -= self._vocab_size_limit
          else:
            self._init_v_in.append(tf.Variable(tf.random_uniform([remain_size_,opts.emb_dim], -0.5 / opts.emb_dim, 0.5 / opts.emb_dim),name="v_in"+str(i)))
            self._init_v_out.append(tf.Variable(tf.zeros([remain_size_,opts.emb_dim]),name="v_in"+str(i)))
            remain_size_ -= remain_size_
          i += 1
      else:
        self._init_v_in.append(tf.Variable(tf.random_uniform([opts.vocab_size,opts.emb_dim], -0.5 / opts.emb_dim, 0.5 / opts.emb_dim),name="v_in"))
        self._init_v_out.append(tf.Variable(tf.zeros([opts.vocab_size,opts.emb_dim]),name="v_out"))
    tf.initialize_all_variables().run()
    if len(self._init_v_in) > 1:
      v_in = tf.get_variable("vin",initializer=tf.concat(0,self._init_v_in))
      v_out = tf.get_variable("vout",initializer=tf.concat(0,self._init_v_out))
    else:
      v_in = tf.get_variable("vin",initializer=self._init_v_in[0])
      v_out = tf.get_variable("vout",initializer=self._init_v_out[0])
    # Global step: []
    global_step = tf.Variable(0, name="global_step")

    self._session.run([v_in.initializer, v_out.initializer, global_step.initializer])
    # Linear learning rate decay.
    words_to_train = float(opts.words_per_epoch * opts.epochs_to_train)
    w_lr = opts.learning_rate * tf.maximum(
        0.0001,
        1.0 - tf.cast(total_words_processed, tf.float32) / words_to_train)
    entities_to_train = float(opts.entities_per_epoch * opts.epochs_to_train)
    e_lr = opts.learning_rate * tf.maximum(
        0.0001,
        1.0 - tf.cast(total_entities_processed, tf.float32) / entities_to_train)
    anchors_to_train = float(opts.anchors_per_epoch * opts.epochs_to_train)
    a_lr = opts.learning_rate * tf.maximum(
        0.0001,
        1.0 - tf.cast(total_anchors_processed, tf.float32) / anchors_to_train)
    # Training nodes.
    inc = global_step.assign_add(1)
    with tf.control_dependencies([inc]):
      w_train = word2vec.neg_train(v_in,
                                 v_out,
                                 w_examples,
                                 w_labels,
                                 w_lr,
      ...
      e_train = word2vec.neg_train(v_in,
                                 v_out,
                                 ...
                                 num_negative_samples=opts.num_samples)
      a_train = word2vec.neg_train(v_in, v_out, a_examples+opts.vocab_word_size, a_labels, a_lr, vocab_count=opts.vocab_word_counts.tolist(), num_negative_samples=opts.num_samples)
  ...
  def build_eval_graph(self):
    ...
    nemb = tf.nn.l2_normalize(self._v_in, 1)
  ...
  def save_embeddings(self):
    opts = self._options
    np.save(os.path.join(opts.save_path,"v_in"),self._v_in.eval())
    np.save(os.path.join(opts.save_path,"v_out"),self._v_out.eval())
  ...
  def load_embeddings_from_file(self):
    opts = self._options
    is_load_suc = True
    tmp_init_vin = np.load(os.path.join(opts.load_path,"v_in.npy"))
    tmp_init_vout = np.load(os.path.join(opts.load_path,"v_out.npy"))
    if len(tmp_init_vin.shape)!=2 or tmp_init_vin.shape[1] != opts.emb_dim or tmp_init_vin.shape[0]!=opts.vocab_size:
      is_load_suc = False
      print("np shape doesn't match!")
    if tmp_init_vin.shape[0] > self._vocab_size_limit:
      remain_size_ = tmp_init_vin.shape[0]
      start_pos_ = 0
      end_pos_ = self._vocab_size_limit
      i = 0
      while remain_size_>0 :
        self._init_v_in.append(tf.Variable(tmp_init_vin[start_pos_:end_pos_,:],name="v_in"+str(i)))
        self._init_v_out.append(tf.Variable(tmp_init_vout[start_pos_:end_pos_,:],name="v_in"+str(i)))
        remain_size_ -= (end_pos_-start_pos_)
        start_pos_ = end_pos_
        if remain_size_ < self._vocab_size_limit:
          end_pos_ = tmp_init_vin.shape[0]
        else:
          end_pos_ += self._vocab_size_limit
        i += 1
    else:
      self._init_v_in.append(tf.Variable(tmp_init_vin,name="v_in0"))
      self._init_v_out.append(tf.Variable(tmp_init_vout,name="v_out0"))
    if is_load_suc:
      self._is_loaded = True
      print("successfully load %d embeddings with %d dimensions in %d variables. " % (tmp_init_vin.shape[0], tmp_init_vin.shape[1], len(self._init_v_in)))
...
def main(_):
  ...
  with tf.Graph().as_default(), tf.Session() as session:
    ...
    endtime = time.time()
    print("The train process costs %d seconds in total." % (endtime-starttime))
